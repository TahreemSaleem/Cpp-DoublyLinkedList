...
public:
	...
	{
		if (name.compare(adr.name) == 0 && city.compare(adr.city) == 0 && ph==adr.ph)
			return true;
		else
			return false;
	}
...
int main(int argc, const char * argv[])
{
	...
	DoublyLinkedList<Address> dlist;

	dlist.insertAtHead(Address("Omar", "ISLAMABAD", 231));
	dlist.insertAtEnd(Address("Faisal", "Karachi", 81));
	dlist.insertAtEnd(Address("Jamal", "Lahore", 22));
	dlist.deleteNode(Address("Omar", "ISLAMABAD", 231));
	dlist.displayList();
